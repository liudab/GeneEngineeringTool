...
export function createGeneEditorWindow(geneId: number): void {
  const existing = geneEditorWindows.get(geneId)
  if (existing && !existing.isDestroyed()) {
    existing.focus()
    return
  }
  ...
  geneEditorWindows.set(geneId, win)
  geneEditorDirtyState.set(geneId, false)

  win.on('close', (e) => {
    const dirty = geneEditorDirtyState.get(geneId)
    if (!dirty) return
    e.preventDefault()
    const result = dialog.showMessageBoxSync(win, {
      type: 'warning',
  ...
  win.on('closed', () => {
    geneEditorWindows.delete(geneId)
    geneEditorDirtyState.delete(geneId)
  })
  ...
  if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
    win.loadURL(`${process.env['ELECTRON_RENDERER_URL']}?geneId=${geneId}&mode=gene-editor`)
  } else {
    win.loadFile(join(__dirname, '../renderer/index.html'), {
      query: { geneId: String(geneId), mode: 'gene-editor' }
    })
  }
}
